...
public class DeleteIrCodeSetRequestValidatorTests
{
    private readonly DeleteIrCodeSetRequestValidator _validator = new();

    [Fact]
    public void Should_Have_No_Errors_When_Request_Is_Valid()
    {
        var request = new DeleteIrCodeSetRequest { Id = 1 };
    ...
    {
        var request = new DeleteIrCodeSetRequest { Id = 0 };
        var result = _validator.TestValidate(request);
