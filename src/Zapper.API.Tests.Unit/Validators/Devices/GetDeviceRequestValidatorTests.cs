...
public class GetDeviceRequestValidatorTests
{
    private readonly GetDeviceRequestValidator _validator = new();

    [Fact]
    public void Should_Have_No_Errors_When_Request_Is_Valid()
    {
        var request = new GetDeviceRequest { Id = 1 };
    ...
    {
        var request = new GetDeviceRequest { Id = 0 };
        var result = _validator.TestValidate(request);
