...
public class DiscoverYamahaDevicesRequestValidatorTests
{
    private readonly DiscoverYamahaDevicesRequestValidator _validator = new();

    [Fact]
    public void Should_Have_No_Errors_When_Request_Is_Valid()
    {
        var request = new DiscoverYamahaDevicesRequest { TimeoutSeconds = 30 };
    ...
    {
        var request = new DiscoverYamahaDevicesRequest { TimeoutSeconds = 0 };
        var result = _validator.TestValidate(request);
    ...
    {
        var request = new DiscoverYamahaDevicesRequest { TimeoutSeconds = 61 };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.TimeoutSeconds)
            .WithErrorMessage("Timeout must be between 1 and 60 seconds");
    }
