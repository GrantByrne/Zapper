...
public class AndroidTvBluetoothController(
    ...
    : IBluetoothDeviceController
{
    ...
    private async Task<bool> HandlePowerCommand(string deviceAddress, DeviceCommand command, CancellationToken cancellationToken)
    {
        ...
        return await hidController.SendKey(deviceAddress, HidKeyCode.Home, cancellationToken);
    }

    private async Task<bool> HandleChannelUp(string deviceAddress, CancellationToken cancellationToken)
    {
        return await hidController.SendKey(deviceAddress, HidKeyCode.PageUp, cancellationToken);
    }

    private async Task<bool> HandleChannelDown(string deviceAddress, CancellationToken cancellationToken)
    {
        return await hidController.SendKey(deviceAddress, HidKeyCode.PageDown, cancellationToken);
    }
