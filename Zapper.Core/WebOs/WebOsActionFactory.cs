    ...
    public class WebOsActionFactory : IWebOsActionFactory
    {
        ...
        public Action Build(string name, Device device)
        {
            var action = name switch
            {
                ...
                WebOsActionKey.Home => 
                    MakeAction(WebOsActionKey.Home, device, (s, _)  => s.Control.SendIntentAsync(ControlService.ControlIntent.Home)),
                WebOsActionKey.Back => 
                    MakeAction(WebOsActionKey.Back, device, (s, _)  => s.Control.SendIntentAsync(ControlService.ControlIntent.Back)),
                WebOsActionKey.Up => 
                    MakeAction(WebOsActionKey.Up, device, (s, _)  => s.Control.SendIntentAsync(ControlService.ControlIntent.Up)),
                WebOsActionKey.Down => 
                    MakeAction(WebOsActionKey.Down, device, (s, _)  => s.Control.SendIntentAsync(ControlService.ControlIntent.Down)),
                WebOsActionKey.Left => 
                    MakeAction(WebOsActionKey.Left, device, (s, _)  => s.Control.SendIntentAsync(ControlService.ControlIntent.Left)),
                WebOsActionKey.Right => 
                    MakeAction(WebOsActionKey.Right, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Right)),
                WebOsActionKey.Red => 
                    MakeAction(WebOsActionKey.Red, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Red)),
                WebOsActionKey.Blue => 
                    MakeAction(WebOsActionKey.Blue, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Blue)),
                WebOsActionKey.Yellow => 
                    MakeAction(WebOsActionKey.Yellow, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Yellow)),
                WebOsActionKey.Green => 
                    MakeAction(WebOsActionKey.Green, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Green)),
                WebOsActionKey.FastForward => 
                    MakeAction(WebOsActionKey.FastForward, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.FastForward)),
                WebOsActionKey.Pause => 
                    MakeAction(WebOsActionKey.Pause, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Pause)),
                WebOsActionKey.Play => 
                    MakeAction(WebOsActionKey.Play, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Play)),
                WebOsActionKey.Rewind => 
                    MakeAction(WebOsActionKey.Rewind, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Rewind)),
                WebOsActionKey.Stop => 
                    MakeAction(WebOsActionKey.Stop, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.Stop)),
                WebOsActionKey.PowerOff => 
                    MakeAction(WebOsActionKey.PowerOff, device, (s, _) => s.Control.SendIntentAsync(ControlService.ControlIntent.PowerOff)),
