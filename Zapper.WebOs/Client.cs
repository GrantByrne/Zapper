...
namespace Zapper.WebOs
{
    public class Client : IDisposable, IClient
    {
        ...
        public Client(IKeyStore keyStore, ILogger<Client> logger) : this(keyStore,
            new Factory<ISocketConnection>(() => new SocketConnection()), logger)
        {
        }
        ...
        public async Task ConnectAsync(string hostName)
        {
            ...
            if (!_socket.IsAlive)
                throw new ConnectionException($"Unable to connect to television at {hostName}.");
            ...
            if (!_mouseSocket.IsAlive)
                throw new ConnectionException($"Unable to connect to television mouse service at {hostName}.");
        }

        public virtual async Task<TResponse> SendCommandAsync<TResponse>(CommandBase command)
